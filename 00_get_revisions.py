...
def get_review_sentences_and_rating(note):
    ...
    elif 'main_review' in note.content:
        review_text = note.content['main_review']
        rating = note.content['recommendation']
    else:
        review_text = note.content['strength_and_weaknesses']
        rating = note.content['recommendation']
    return [
        sent.text
...
def retrieve_forum(forum, conference, output_dir):
    ...
    for note in GUEST_CLIENT.get_all_notes(forum=forum.id):
        ...
        if (note.replyto == note.forum and
            ...
            or "review" in note.content
            or "Official_Review" in note.invitation)):
            # ICLR 2018 - 2021 have review as a field
            # ICLR 2022 has main_review as a field
            # Others have separate fields
...
def main():
    ...
    print(INVITATIONS[args.conference])
    print(len(forum_notes))
    success_count = 0
